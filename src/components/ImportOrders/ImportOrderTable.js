...
const ImportOrderTable = ({
  ...
  const getStatusLabel = (statusValue) => {
    return (
      importOrderStatusOptions.find((opt) => opt.value === statusValue)
        ?.label || statusValue
    );
  };
          ...
          {importOrders && importOrders.length > 0 ? (
            importOrders.map((order) => (
              <tr key={order.id}>
                ...
                <td data-label="NCC:">
                  {order.supplier_id.name || order.supplier_id}
                </td>
                <td data-label="Ngày Tạo:">
                  {new Date(
                    order.import_date || order.created_at
                ...
                <td data-label="Trạng Thái:" className={styles.statusCell}>
                  ...
                  order.status !== "cancelled" ? (
                    <Select
                      ...
                      value={importOrderStatusOptions.find(
                        (opt) => opt.value === order.status
                      )}
                      onChange={(selectedOption) =>
                        onUpdateStatus(order.id, selectedOption.value)
                      }
                      ...
                      styles={{
                        ...
                        singleValue: (base) => ({
                          // Style cho text trạng thái hiển thị
                          ...base,
                ...
                <td data-label="Hành Động:" className={styles.actionsCell}>
                  ...
                  {onDelete &&
                    ...
                    order.status !== "cancelled" && (
                      <button
                        onClick={() => onDelete(order._id,order)}
